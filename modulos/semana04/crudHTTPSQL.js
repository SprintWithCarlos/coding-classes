...
app.post('/books', (req, res) => {
  ...
  mysqlConnection.query('INSERT INTO library SET ?', book, (err, result) => {
    if (err) throw err;
    res.status(201).json({
...
app.put('/books/:id', (req, res) => {
  ...
  mysqlConnection.query('UPDATE library SET ? WHERE book_id = ?', param, (err, result) => {
    if (err) throw err;
    res.status(200).json({
...
app.delete('/books/:id', async (req, res) => {
  const { id } = req.params;
  const result = await mysqlConnection.query('DELETE FROM library WHERE book_id = ?', id);
  res.status(200).json({
